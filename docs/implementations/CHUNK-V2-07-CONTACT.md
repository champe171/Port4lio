# CHUNK-V2-07 — Contact Section Visual Redesign
...
## Current state
- Cream background
- Heading "Contact" in red
- Label/value rows (Phone, Email, LinkedIn) with thin top borders

## Target (Canva reference)
- **Cream background** (same — this section keeps cream)
- Giant **"LET'S WORK TOGETHER"** in bold RED text spanning nearly full width
- Asterisk decorator below the giant heading
- Contact info at the bottom: phone, email, LinkedIn displayed as **icon + value** horizontal row (not label/value stacked)
- Section number `06` top-right

---

## Full replacement for `components/portfolio/ContactSection.tsx`
...
1. Giant `"LET'S WORK TOGETHER"` in `text-portfolio-red` with huge font size
2. Asterisk after the heading (using existing `.asterisk` class — red color, large)
3. Contact links use icon + label layout (icon in circle border, hover fills red)
4. Three inline SVG icons: phone, email, LinkedIn (no external icon library needed)
5. `LET&apos;S` for the apostrophe (JSX safe)
6. Removed `ContactRow` component, replaced with `ContactLink`

## Verification
- "LET'S WORK TOGETHER" renders in large bold red text
- Asterisk visible below the heading
- Phone, email, LinkedIn show as icon circles + text in a horizontal row
- Hovering a contact link: icon fills red, text turns red
- No TypeScript errors (React.ReactNode import not needed — JSX.Element inferred)
