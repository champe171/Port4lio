# CHUNK-V2-04 — Education Section Visual Redesign
...
## Current state
- Cream background
- 2-col grid: education entries left, certifications right
- Black top-border cards, gray date text

## Target (Canva reference)
- **Full BLACK background** (`bg-portfolio-black`)
- **RED heading** "EDUCATION & CERTIFICATION" (white text not red — actually on black bg, use white for body, red for accent)
- **Asterisk bullets** in red before school names and cert names
- Section number `03` top-right (white)
- All text white on black background

---

## Full replacement for `components/portfolio/EducationSection.tsx`
...
1. `bg-portfolio-black` on section (was `bg-portfolio-cream`)
2. All text white: `text-white`, `text-white/60`, `text-white/50`
3. Heading `"EDUCATION & CERTIFICATION"` in `text-portfolio-red` (red on black = high contrast)
4. Red `*` asterisk before each school name and cert name
5. Section number `text-white/30`
6. Border: `border-white/10` (subtle on black)
7. No `SectionWrapper` import — inline motion divs with `isInView` passed as prop
8. `&amp;` for the `&` in the heading (JSX safe)

## Verification
- Section has solid black background
- "EDUCATION & CERTIFICATION" heading is red
- Each education entry has a red `*` bullet before school name
- Each certification has a red `*` bullet before cert name
